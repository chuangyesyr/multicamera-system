...
Tracking_Consumption = 0.001;                                                % Energy consumption when camera is busy
x_min = 0.1;
x_max = 1.9;
y_min = 0.1;
y_max = 1.9;
...
switch InitialE
    case 1
        ...
        savefile_RE = './%s/Initial_REnergy';
        ssfileRE = sprintf(savefile_RE, Filename);
        save(ssfileRE, 'Initial_REnergy');
        save('Initial_REnergy', 'Initial_REnergy');
...
handles.REnergy = Initial_REnergy;
REnergy = Initial_REnergy;
...
handles.Table2 = zeros(nC+1, nO+2);
...
%     TableOcc = handles.TableOcc;
    ...
    for k = 1:nO
        ...
        if (~isempty(Camera_Index1))
            ...
            angles_a(Camera_Index1, k) = Angles;
            Angles_Diff = abs(Angles - handles.coordinates_camera(3, Camera_Index1));
            angles_Diff = Angles - handles.coordinates_camera(3, Camera_Index1);
    ...
    if (j > 2)                                                          % flag 7 trigger (An objcet is moving out from FOV of the static tracking camera)
        ...
        if (j > 3)
            ...
            Final_Decision = (Angle_Decision + Distance_Decision).*TT1;
...
%                         utility_obj_idlecam = (W_ME*energy_idlecam + W_MD*(1-dis_obj_idlecam/max(dis_obj_idlecam))).*speed_ind;  
        ...
        if (flag8 == 1)
            count_send_resource(nSC+ind_MC_tr) = count_send_resource(nSC+ind_MC_tr) + 2;
            index_idle_M1 = find(situation_Mcam == 0);
            count_send_resource(nSC+index_idle_M1) = count_send_resource(nSC+index_idle_M1) + 1;
            idle_M_len = length(index_idle_M1);
            ccc = 0;
            index_idle_M = [];
            for i_idle = 1:idle_M_len
                if (Energy_Left(index_idle_M1(i_idle)) > EMC_Thr)
                    ccc = ccc + 1;
            ...
            if (~isempty(index_idle_M))
                ...
                speed_ind_M(speed_ind_M > speed_lim) = 1;
                speed_ind_M = 1 - speed_ind_M;
...
%                 elseif (~isempty(index_cam_tra) && index_cam_tra > nSC && (Table2(index_cam_tra, index_obj_assigned) == 0 || sum(Table1(1:nC, index_obj_assigned)) > 1 || Table1(index_cam_tra, index_obj_assigned) == 1))    %% none of the camera see obj, any other camera sees the obj
    ...
    if (j > 2 && Mobile_flag == 1)
        for i = 1:nMC
            ...
            if (~isempty(obj_tracked) && Energy_Left(i+nSC) ~= 0)                 % Mobile camera is tracking
...
%                 speed1 = max((dist - 1/2*FOVlen)*factor_speed*2 + obj_speed * obj_tracked, 0);  
            ...
            elseif (idlecam_assigned(i, 1) == 1 && Energy_Left(i+nSC) ~= 0)   
                x1(i, j) = x1(i, j-1) + speed_act(i) * cos(direction(i));
                y1(i, j) = y1(i, j-1) + speed_act(i) * sin(direction(i));
...
%     Energy_cons * [count_send_feature; count_send_resource; count_send_list; count_send_load];
    ...
    handles.REnergy = [handles.REnergy; REnergy];
